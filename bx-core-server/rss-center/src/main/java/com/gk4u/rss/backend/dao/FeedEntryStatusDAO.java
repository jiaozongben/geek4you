...
import com.gk4u.rss.backend.FixedSizeSortedSet;
...
public class FeedEntryStatusDAO extends GenericDAO<FeedEntryStatus> {
	...
	private FeedEntryStatus handleStatus(User user, FeedEntryStatus status, FeedSubscription sub, FeedEntry entry) {
		if (status == null) {
			Date unreadThreshold = config.getUnreadThreshold();
	...
	public List<FeedEntryStatus> findStarred(User user, Date newerThan, int offset, int limit, UserSettings.ReadingOrder order, boolean includeContent) {
		...
		if (order == UserSettings.ReadingOrder.asc) {
			query.orderBy(status.entryUpdated.asc(), status.id.asc());
		} else if (order == UserSettings.ReadingOrder.desc) {
			query.orderBy(status.entryUpdated.desc(), status.id.desc());
		} else if (order == UserSettings.ReadingOrder.abc) {
		...
		setTimeout(query, config.getQueryTimeout());
	...
	private JPAQuery<FeedEntry> buildQuery(User user, FeedSubscription sub, boolean unreadOnly, List<FeedEntryKeyword> keywords,
										   Date newerThan, int offset, int limit, UserSettings.ReadingOrder order, FeedEntryStatus last, String tag) {
		...
		if (CollectionUtils.isNotEmpty(keywords)) {
			...
			for (FeedEntryKeyword keyword : keywords) {
				...
				if (keyword.getMode() == FeedEntryKeyword.Mode.EXCLUDE) {
					or.not();
				}
				query.where(or);
			}
		...
		if (last != null) {
			if (order == UserSettings.ReadingOrder.desc) {
				query.where(entry.updated.gt(last.getEntryUpdated()));
			} else if (order == UserSettings.ReadingOrder.asc) {
		...
		} else if (order != null && (order == UserSettings.ReadingOrder.abc || order == UserSettings.ReadingOrder.zyx)) {
			query.join(entry.content, content);
		}
	...
	public List<FeedEntryStatus> findBySubscriptions(User user, List<FeedSubscription> subs, boolean unreadOnly,
													 List<FeedEntryKeyword> keywords, Date newerThan, int offset, int limit, UserSettings.ReadingOrder order, boolean includeContent,
		...
		} else if (order == UserSettings.ReadingOrder.zyx) {
