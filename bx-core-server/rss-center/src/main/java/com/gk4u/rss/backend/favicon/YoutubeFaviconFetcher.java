package com.gk4u.rss.backend.favicon;
...
public class YoutubeFaviconFetcher extends AbstractFaviconFetcher {
	...
	public Favicon fetch(Feed feed) {
		...
		String googleAuthKey = config.getGoogleAuthKey();
		...
		try {
			...
			HttpGetter.HttpResult iconResult = getter.getBinary(thumbnail.getUrl(), TIMEOUT);
